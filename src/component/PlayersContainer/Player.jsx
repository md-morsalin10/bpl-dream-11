...
const Player = ({ player, setCoin, coin, selectedPlayers, setSelectedPlayers }) => {
    ...
    return (
        <div className='rounded-2xl p-5 space-y-2 shadow-2xl card bg-base-100 border border-gray-200 transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 cursor-pointer'>
            ...
            <div className='flex justify-between items-center'>
                ...
                <p className='px-4 py-2 bg-gray-200 rounded-lg font-medium'>{player.playerType}</p>
            </div>
            ...
            <div className='flex justify-between'>
                ...
                <button onClick={handleBtn} className={`btn ${isSelected ? 'text-green-600':'btn-primary'}`} disabled={isSelected ? true : false}>{isSelected ? 'Selected' : 'Choose Player'}</button>
            </div>
        </div>
    );
};
